# ===============================================================
#   20_GEX_pseudobulk_DESeq2_endothelial_subtypes_HA_vs_other.R
# ===============================================================

# Pseudobulk differential gene expression between HA and all other
# samples for each endothelial subtype.
# The sample is used as the statistical unit and raw RNA counts are
# summed within each sample x endothelial subtype group.
...
CELLTYPE_COL <- "endothelial_subtype"
...
colnames(meta_df)[colnames(meta_df) == CELLTYPE_COL] <- "endothelial_subtype"
...
meta_df$endothelial_subtype <- as.character(meta_df$endothelial_subtype)

meta_df <- meta_df[
  !is.na(meta_df$condition) &
    !is.na(meta_df$sample_id) & meta_df$sample_id != "" &
    !is.na(meta_df$endothelial_subtype) & meta_df$endothelial_subtype != "",
  ,
  drop = FALSE
]

cat("Cells used for endothelial pseudobulk analysis:", nrow(meta_df), "\n")
...
for (ct in endothelial_subtypes) {
  cat("\n------------------------------------------------------------\n")
  cat("Endothelial subtype:", ct, "\n")
  safe_ct <- sanitize_label(ct)

  cells_ct <- rownames(meta_df)[meta_df$endothelial_subtype == ct]
  ...
  for (s in rownames(coldata)) {
    ...
    pb_counts[, s] <- Matrix::rowSums(counts_sample)
    detected_fraction[, s] <- Matrix::rowSums(counts_sample > 0) / length(cells_sample)
  }
  ...
  write.csv(
    sig_df,
    file.path(RES_DIR, paste0("deseq2_HA_vs_other_", safe_ct, "_significant.csv")),
    row.names = FALSE
  )
  ...
  write.csv(
    normalized_counts,
    file.path(RES_DIR, paste0("normalized_counts_", safe_ct, ".csv"))
  )
...
write.csv(
  summary_df,
  file.path(RES_DIR, "deseq2_pseudobulk_summary_by_endothelial_subtype.csv"),
  row.names = FALSE
)
...
for (res_file in tested_result_files) {
  ...
  safe_ct <- sub("^deseq2_HA_vs_other_", "", basename(res_file))
  safe_ct <- sub("\\.csv$", "", safe_ct)
  summary_safe_ct <- sanitize_label(summary_df$endothelial_subtype)
  cell_type_here <- summary_df$endothelial_subtype[summary_safe_ct == safe_ct][1]

  res_df$endothelial_subtype <- cell_type_here
...
cumulative_df$plot_group[cumulative_df$is_significant] <- cumulative_df$endothelial_subtype[cumulative_df$is_significant]
cumulative_df$plot_group <- factor(cumulative_df$plot_group, levels = c("Not significant", names(ENDOTHELIAL_SUBTYPE_COLORS)))

cumulative_colors <- c("Not significant" = "grey75", ENDOTHELIAL_SUBTYPE_COLORS)
...
p_cumulative_volcano <- ggplot(cumulative_df, aes(x = log2FoldChange, y = neg_log10_padj, color = plot_group)) +
  ...
  xlim(-LOG2FC_PLOT_LIMIT, LOG2FC_PLOT_LIMIT) +
  labs(
    title = "Dysregulated genes in HA endothelial subtypes",
    x = "log2 fold change",
    y = "-log10 adjusted p-value",
    color = NULL
  ) +
...
ggsave(
  filename = file.path(VOLCANO_DIR, "volcano_cumulative_HA_vs_other_endothelial_subtypes.png"),
...
# Save a summary of significant DEGs for each endothelial subtype
...
write.csv(
  deg_summary_df,
  file.path(RES_DIR, "deseq2_significant_genes_summary_endothelial_subtypes.csv"),
  row.names = FALSE
)

# Bubble heatmap of significant iron-related DEGs across endothelial subtypes
...
for (res_file in tested_result_files) {
  ...
  safe_ct <- sub("\\.csv$", "", safe_ct)
  summary_safe_ct <- sanitize_label(summary_df$endothelial_subtype)
  subtype_here <- summary_df$endothelial_subtype[summary_safe_ct == safe_ct][1]
  ...
  res_df$endothelial_subtype <- subtype_here
  iron_bubble_rows[[subtype_here]] <- res_df[, c("endothelial_subtype", "gene", "log2FoldChange", "padj"), drop = FALSE]
}
...
if (length(iron_bubble_rows) > 0) {
  ...
  subtype_order <- summary_df$endothelial_subtype[summary_df$status == "tested"]
  subtype_order <- subtype_order[subtype_order %in% unique(iron_bubble_df$endothelial_subtype)]
  iron_bubble_df$endothelial_subtype <- factor(iron_bubble_df$endothelial_subtype, levels = subtype_order)
  ...
  p_iron_bubble <- ggplot(iron_bubble_df, aes(x = endothelial_subtype, y = gene)) +
    geom_point(aes(size = neg_log10_padj, fill = log2FoldChange), shape = 21, color = "black", stroke = 0.3) +
...
cat("\n============================================================\n")
cat("Endothelial subtype pseudobulk DESeq2 analysis complete.\n")
cat("Results dir : ", RES_DIR, "\n")
cat("Figures dir : ", FIG_DIR, "\n")
cat("Summary     : ", file.path(RES_DIR, "deseq2_pseudobulk_summary_by_endothelial_subtype.csv"), "\n")
cat("DEG summary : ", file.path(RES_DIR, "deseq2_significant_genes_summary_endothelial_subtypes.csv"), "\n")
if (length(iron_bubble_rows) > 0) {
