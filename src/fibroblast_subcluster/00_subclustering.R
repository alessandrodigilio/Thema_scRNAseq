###################################################
### Destructive lining fibroblast subclustering ###
###################################################
...
fig_dir <- file.path(fibroblast_figures_dir, "destructive_lining_fibroblast_subclustering")
res_dir <- file.path(fibroblast_results_dir, "destructive_lining_fibroblast_subclustering")
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)
dir.create(fig_dir, recursive = TRUE, showWarnings = FALSE)
dir.create(res_dir, recursive = TRUE, showWarnings = FALSE)
output_subset_object <- file.path(out_dir, "destructive_lining_fibroblasts_subclustered.rds")
output_full_object <- file.path(out_dir, "annotated_destructive_lining_fibroblast_subclusters.rds")
...
# save
write.csv(cluster_summary, file.path(res_dir, "destructive_lining_fibroblast_cluster_resolution_summary.csv"), row.names = FALSE)

# select res 0.3 for subclustering
...
# save
write.csv(markers, file.path(res_dir, "all_markers_destructive_lining_fibroblast_subclusters.csv"), row.names = FALSE)
...
top_markers_save <- markers %>%
  ...
  ungroup()
# save
write.csv(top_markers_save, file.path(res_dir, "top_markers_per_destructive_lining_fibroblast_subcluster.csv"), row.names = FALSE)
...
cat("\nTop markers used for destructive lining fibroblast subcluster review:\n")
...
ggsave(file.path(fig_dir, "dotplot_top_markers_destructive_lining_fibroblast_subclusters.png"), p_marker_dot, width = 12, height = 6, dpi = 600)

# dotplot focused on lining/remodeling fibroblast markers
fibro_features <- destructive_lining_fibroblast_marker_features[
  destructive_lining_fibroblast_marker_features %in% rownames(obj)
]
p_fibro_dot <- DotPlot(
  object = obj,
...
ggsave(file.path(fig_dir, "dotplot_lining_remodeling_markers_destructive_lining_fibroblast_subclusters.png"), p_fibro_dot, width = 14, height = 6, dpi = 600)

# dotplot focused on iron-related genes
iron_features <- destructive_lining_fibroblast_iron_features[
  destructive_lining_fibroblast_iron_features %in% rownames(obj)
]
p_iron_dot <- DotPlot(
  object = obj,
...
ggsave(file.path(fig_dir, "dotplot_iron_related_markers_destructive_lining_fibroblast_subclusters.png"), p_iron_dot, width = 14, height = 6, dpi = 600)
