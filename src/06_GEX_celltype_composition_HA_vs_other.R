# ===============================================================
#   06_GEX_celltype_composition_HA_vs_other.R
# ===============================================================

# Test whether final atlas cell type proportions differ between HA
# and other samples using the sample as the statistical unit.
...
# Count cells and calculate cell type fractions per sample
...
write.csv(
  ...
  file.path(RES_DIR, "celltype_fraction_by_sample.csv"),
  row.names = FALSE
)

# Compare HA and other fractions across samples for each cell type
stat_rows <- list()
...
write.csv(
  stat_df,
  file.path(RES_DIR, "celltype_fraction_HA_vs_other_wilcoxon.csv"),
  row.names = FALSE
)

top_diff_df <- stat_df[order(abs(stat_df$diff_mean_ha_minus_other), decreasing = TRUE), , drop = FALSE]
cat("Top cell types ranked by mean difference in fraction (first 6 rows):\n")
print(head(top_diff_df))

write.csv(
  top_diff_df,
  file.path(RES_DIR, "celltype_fraction_HA_vs_other_ranked_by_effect.csv"),
  row.names = FALSE
)

num_significant <- sum(!is.na(stat_df$padj_bh) & stat_df$padj_bh < FDR_THR)
cat("Number of cell types with significant fraction differences at FDR < ", FDR_THR, ": ", num_significant, "\n", sep = "")
cat("Significant cell types:\n")
...
ggsave(
  filename = file.path(FIG_DIR, "celltype_fraction_HA_vs_other_boxplots.png"),
  plot = p_box,
  width = 15,
  height = 10,
  dpi = 600
