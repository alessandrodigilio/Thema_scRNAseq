###############################################
### endothelial subcluster helper functions ###
###############################################

# apply final endothelial subtype labels to a subclustered object
add_endothelial_subtype_labels <- function(obj, cluster_col = "endothelial_subcluster", label_col = "endothelial_subtype") {
  cluster_ids <- as.character(obj@meta.data[[cluster_col]])
  labels <- unname(endothelial_subcluster_labels[cluster_ids])
  labels[is.na(labels)] <- "Unknown"
  obj@meta.data[[label_col]] <- factor(labels, levels = unname(endothelial_subcluster_labels))
  obj@meta.data[[label_col]] <- droplevels(obj@meta.data[[label_col]])
  obj
}

# get colors for the endothelial subtypes present in the object
get_endothelial_colors <- function(labels) {
  labels <- unique(as.character(labels))
  endothelial_subtype_colors[labels]
}
...
# convert the endothelial marker list into a simple subtype/gene table
make_endothelial_marker_table <- function(obj) {
  marker_rows <- list()

  for (subtype in names(marker_genes_endothelial)) {
    genes <- marker_genes_endothelial[[subtype]]
    genes <- genes[genes %in% rownames(obj)]
    if (length(genes) == 0) next
    marker_rows[[subtype]] <- data.frame(endothelial_subtype = subtype, gene = genes, stringsAsFactors = FALSE)
  }

  do.call(rbind, marker_rows)
}

# compute average expression and percent expression for an endothelial marker dotplot
build_endothelial_marker_dotplot_data <- function(object, features, group_col) {
...
# plot an endothelial marker bubble plot
plot_endothelial_marker_dotplot <- function(dotplot_df, low_col, mid_col, high_col) {
...
# plot sample-level mean expression for one gene in one endothelial subtype
plot_gene_by_sample_in_endothelial_subtype <- function(obj,
                                                       gene,
                                                       endothelial_subtype,
                                                       group_col,
                                                       sample_col,
                                                       ...
                                                       group_levels,
                                                       group_colors) {
  cells_use <- rownames(obj@meta.data)[obj@meta.data[[subtype_col]] == endothelial_subtype]
  ...
  p <- ggplot(sample_df, aes(x = condition, y = expression, fill = condition)) +
    ...
    labs(title = paste0(gene, " in ", endothelial_subtype), x = NULL, y = "Mean norm. expression per sample") +
...
# draw average ferroptosis gene expression by endothelial subtype and condition
make_endothelial_ferroptosis_heatmap <- function(obj, genes_use, label_col) {
  ...
  heatmap_df$endothelial_subtype <- factor(vapply(heatmap_groups, `[`, character(1), 1), levels = subtype_levels)
  ...
  p <- ggplot(heatmap_df, aes(x = HA_vs_other, y = gene, fill = z_score)) +
    ...
    facet_grid(cols = vars(endothelial_subtype), scales = "free_x", space = "free_x", labeller = labeller(endothelial_subtype = wrap_each_word)) +
  ...
  list(plot = p, n_subtypes = length(unique(heatmap_df$endothelial_subtype)), n_genes = nrow(scaled_expr))
}
