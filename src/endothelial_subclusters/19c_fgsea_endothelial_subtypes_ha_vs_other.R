...
# run ranked GSEA with fgsea on endothelial-subtype pseudobulk DESeq2 results.
...
pseudobulk_res_dir <- file.path(results_dir, "pseudobulk_deseq2_endothelial_subtypes")
fgsea_res_dir <- file.path(results_dir, "fgsea_pseudobulk_endothelial_subtypes_HA_vs_other")
...
fgsea_fig_dir <- file.path(figures_dir, "fgsea_pseudobulk_endothelial_subtypes_HA_vs_other")
...
# check endothelial-subtype pseudobulk summary.
...
for (i in seq_len(nrow(summary_df))) {
  subtype_here <- summary_df$endothelial_subtype[i]
  ...
  cat("\n============================================================\n")
  cat("Running fgsea for endothelial subtype:", subtype_here, "\n")
  ...
  fgsea_res$endothelial_subtype <- subtype_here
  ...
  fgsea_res <- fgsea_res[
    ,
    c(
      "endothelial_subtype", "database", "pathway_name", "is_iron_related",
...
if (length(all_sig_results) > 0) {
  ...
  write.csv(all_sig_df, file.path(fgsea_res_dir, "fgsea_HA_vs_other_all_endothelial_subtypes_significant.csv"), row.names = FALSE)
}

if (length(all_iron_sig_results) > 0) {
  all_iron_sig_df <- do.call(rbind, all_iron_sig_results)
  rownames(all_iron_sig_df) <- NULL
  write.csv(all_iron_sig_df, file.path(fgsea_res_dir, "fgsea_HA_vs_other_all_endothelial_subtypes_iron_related_significant.csv"), row.names = FALSE)
}

cat("\n============================================================\n")
cat("Endothelial-subtype fgsea ranked GSEA complete.\n")
