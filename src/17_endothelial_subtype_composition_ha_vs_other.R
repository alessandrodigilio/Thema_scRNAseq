####################################################
### Endothelial subtype composition: HA vs other ###
####################################################

# test whether endothelial subtype proportions differ between HA and
...
write.csv(composition_df[, c("sample_id", "condition", "cell_type", "n_cells", "sample_total_cells", "fraction")], file.path(res_dir, "endothelial_subtype_fraction_by_sample.csv"), row.names = FALSE)
...
write.csv(stat_df, file.path(res_dir, "endothelial_subtype_fraction_HA_vs_other_wilcoxon.csv"), row.names = FALSE)

top_diff_df <- stat_df[order(abs(stat_df$diff_mean_ha_minus_other), decreasing = TRUE), , drop = FALSE]
cat("Top endothelial subtypes ranked by mean difference in fraction (first 6 rows):\n")
print(head(top_diff_df))

write.csv(top_diff_df, file.path(res_dir, "endothelial_subtype_fraction_HA_vs_other_ranked_by_effect.csv"), row.names = FALSE)

num_significant <- sum(!is.na(stat_df$padj_bh) & stat_df$padj_bh < fdr_thr)
cat("Number of endothelial subtypes with significant fraction differences at FDR < ", fdr_thr, ": ", num_significant, "\n", sep = "")
cat("Significant endothelial subtypes:\n")
...
p_box <- ggplot(composition_df, aes(x = condition, y = fraction, color = condition)) +
  ...
  labs(x = NULL, y = "Endothelial subtype fraction", fill = NULL) +
...
ggsave(
  filename = file.path(fig_dir, "endothelial_subtype_fraction_HA_vs_other_boxplots.png"),
  plot = p_box,
  width = 12,
  height = 9,
  dpi = 600
...
cat("\n============================================================\n")
cat("HA vs other endothelial composition analysis complete.\n")
cat("Results dir : ", res_dir, "\n")
cat("Figures dir : ", fig_dir, "\n")
cat("Main table  : ", file.path(res_dir, "endothelial_subtype_fraction_HA_vs_other_wilcoxon.csv"), "\n")
cat("============================================================\n")
