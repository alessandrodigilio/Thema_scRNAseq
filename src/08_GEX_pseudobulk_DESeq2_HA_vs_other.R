...
if (!file.exists(INPUT_OBJECT)) {
  stop("Missing final annotated object: ", INPUT_OBJECT)
}
...
colnames(meta_df)[colnames(meta_df) == CELLTYPE_COL] <- "cell_type"
...
cat("Cells used for pseudobulk analysis:", nrow(meta_df), "\n")
cat("Samples by HA vs other:\n")
print(table(meta_df$sample_id, meta_df$condition))
cat("Minimum cells per sample x cell type:", MIN_CELLS_PER_SAMPLE, "\n")
...
for (ct in cell_types) {
  ...
  cells_ct <- rownames(meta_df)[meta_df$cell_type == ct]
  ...
  # Keep genes with counts in enough pseudobulk samples and that are also
  # detected in a meaningful fraction of cells before running DESeq2.
  ...
  # Sample-level QC before reading DE results.
  # The HBC pseudobulk scRNA-seq tutorial uses rlog/blind=TRUE to inspect
  # whether condition is a major source of pseudobulk variation.
...
write.csv(
  summary_df,
  file.path(RES_DIR, "deseq2_pseudobulk_summary_by_celltype.csv"),
  row.names = FALSE
)
...
for (res_file in tested_result_files) {
  ...
  safe_ct <- sub("^deseq2_HA_vs_other_", "", basename(res_file))
  safe_ct <- sub("\\.csv$", "", safe_ct)
  summary_safe_ct <- gsub("[^A-Za-z0-9]+", "_", summary_df$cell_type)
  summary_safe_ct <- gsub("^_+|_+$", "", summary_safe_ct)
  cell_type_here <- summary_df$cell_type[summary_safe_ct == safe_ct][1]

  res_df$cell_type <- cell_type_here
...
p_cumulative_volcano <- ggplot(cumulative_df, aes(x = log2FoldChange, y = neg_log10_padj, color = plot_group)) +
  ...
  xlim(-LOG2FC_PLOT_LIMIT, LOG2FC_PLOT_LIMIT) +
  labs(
    title = "Dysregulated genes in HA synovium",
    x = "log2 fold change",
    y = "-log10 adjusted p-value",
    color = NULL
  ) +
...
ggsave(
  filename = file.path(VOLCANO_DIR, "volcano_cumulative_HA_vs_other.png"),
...
# How many significant genes for each cell type?
...
write.csv(
  deg_summary_df,
  file.path(RES_DIR, "deseq2_significant_genes_summary.csv"),
  row.names = FALSE
)
