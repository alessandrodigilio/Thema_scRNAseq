# ---------------------------------------
# 14_GEX_ferroptosis_macrophage_subtypes_HA_vs_other.R
# Ferroptosis analysis in macrophage subtypes: HA vs other
# ---------------------------------------
...
if (!file.exists(INPUT_OBJECT)) {
  stop("Missing macrophage subset object: ", INPUT_OBJECT)
}
...
if (!CLUSTER_COL %in% colnames(obj@meta.data)) {
  stop("Macrophage subcluster column not found in metadata: ", CLUSTER_COL)
}
...
# Add macrophage subtype labels from the finalized annotation
...
if (length(genes_use) == 0) {
  stop("No ferroptosis genes from Excel were found in the macrophage subset")
}
...
summary_subtype <- aggregate(
  ferroptosis_score ~ macrophage_subtype + HA_vs_other,
  data = obj@meta.data,
  FUN = function(x) c(
    mean = mean(x, na.rm = TRUE),
    median = median(x, na.rm = TRUE),
...
summary_subtype_out <- data.frame(
  macrophage_subtype = summary_subtype$macrophage_subtype,
...
write.csv(
  summary_subtype_out,
  file.path(RES_DIR, "ferroptosis_score_by_macrophage_subtype_HA_vs_other.csv"),
  row.names = FALSE
)

if ("sample_id" %in% colnames(obj@meta.data)) {
  summary_sample <- aggregate(
    ferroptosis_score ~ sample_id + macrophage_subtype + HA_vs_other,
    data = obj@meta.data,
    FUN = function(x) c(
      mean = mean(x, na.rm = TRUE),
      median = median(x, na.rm = TRUE),
  ...
  summary_sample_out <- data.frame(
    ...
    macrophage_subtype = summary_sample$macrophage_subtype,
  ...
  write.csv(
    summary_sample_out,
    file.path(RES_DIR, "ferroptosis_score_by_sample_macrophage_subtype_HA_vs_other.csv"),
    row.names = FALSE
  )
}

# UMAP of ferroptosis score in macrophage subset
...
ggsave(
  filename = file.path(FIG_DIR, "umap_ferroptosis_score_macrophage_subtypes.png"),
  plot = p_score,
  width = 8,
  height = 7,
  dpi = 600
...
ggsave(
  filename = file.path(FIG_DIR, "umap_ferroptosis_score_macrophage_subtypes_split_HA_vs_other.png"),
  plot = p_score_split,
  width = 14,
  height = 7,
  dpi = 600
...
ggsave(
  filename = file.path(FIG_DIR, "violin_ferroptosis_score_macrophage_subtypes_HA_vs_other.png"),
  plot = p_vln_group,
  width = 6,
  height = 6,
  dpi = 600
...
# Violin by macrophage subtype split HA vs other
...
p_vln_subtype <- ggplot(vln_df_subtype, aes(x = macrophage_subtype, y = ferroptosis_score, fill = HA_vs_other)) +
  geom_violin(
    trim = TRUE,
    scale = "width",
...
ggsave(
  filename = file.path(FIG_DIR, "violin_ferroptosis_score_by_macrophage_subtype_HA_vs_other.png"),
  plot = p_vln_subtype,
  width = 14,
  height = 7,
  dpi = 600
...
# Heatmap of ferroptosis genes by macrophage subtype and HA status
...
if (length(dot_features) > 0) {
  ...
  if (length(heatmap_subtypes) > 0) {
    ...
    if (ncol(avg_expr) > 0) {
      ...
      heatmap_df$macrophage_subtype <- factor(vapply(heatmap_groups, `[`, character(1), 1), levels = heatmap_subtypes)
      ...
      p_heatmap <- ggplot(heatmap_df, aes(x = HA_vs_other, y = gene, fill = z_score)) +
        ...
        facet_grid(
          cols = vars(macrophage_subtype),
          scales = "free_x",
          space = "free_x",
          labeller = labeller(macrophage_subtype = wrap_each_word)
        ) +
      ...
      ggsave(
        filename = file.path(FIG_DIR, "heatmap_ferroptosis_genes_by_macrophage_subtype_HA_vs_other.png"),
        plot = p_heatmap,
        width = max(12, 1.15 * length(unique(heatmap_df$macrophage_subtype))),
