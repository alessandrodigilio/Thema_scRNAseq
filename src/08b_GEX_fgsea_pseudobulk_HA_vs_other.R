# ===============================================================
#   08b_GEX_fgsea_pseudobulk_HA_vs_other.R
# ===============================================================

# Run ranked GSEA with fgsea on pseudobulk DESeq2 results.
# Genes are ranked by the DESeq2 Wald statistic when available.
...
FGSEA_RES_DIR <- file.path(RESULTS_DIR, "fgsea_pseudobulk_HA_vs_other")
...
FGSEA_FIG_DIR <- file.path(FIGURES_DIR, "fgsea_pseudobulk_HA_vs_other")
...
for (i in seq_len(nrow(summary_df))) {
  ...
  safe_ct <- sanitize_label(cell_type_here)
  deg_file <- file.path(PSEUDOBULK_RES_DIR, paste0("deseq2_HA_vs_other_", safe_ct, ".csv"))

  if (!file.exists(deg_file)) {
    cat("Missing DESeq2 result for ", cell_type_here, ": ", deg_file, "\n", sep = "")
    next
  }

  cat("\n============================================================\n")
  cat("Running fgsea for:", cell_type_here, "\n")
  ...
  fgsea_res$cell_type <- cell_type_here
  ...
  fgsea_res <- fgsea_res[
    ,
    c(
      "cell_type", "database", "pathway_name", "is_iron_related",
      "pathway", "pval", "padj", "log2err", "ES", "NES", "size", "leadingEdge"
    ),
    drop = FALSE
  ]

  all_fgsea_results[[cell_type_here]] <- fgsea_res

  sig_res <- fgsea_res[!is.na(fgsea_res$padj) & fgsea_res$padj < PADJ_THR, , drop = FALSE]
  all_sig_results[[cell_type_here]] <- sig_res

  cat("Significant pathways at padj < ", PADJ_THR, ": ", nrow(sig_res), "\n", sep = "")
  ...
  for (db in names(MSIGDB_COLLECTIONS)) {
    ...
    write.csv(
      sig_db,
      file.path(FGSEA_TABLE_DIR, paste0("fgsea_HA_vs_other_", safe_ct, "_", db, "_significant.csv")),
      row.names = FALSE
    )
  }

  iron_sig_res <- sig_res[sig_res$is_iron_related, , drop = FALSE]
  all_iron_sig_results[[cell_type_here]] <- iron_sig_res

  if (nrow(iron_sig_res) > 0) {
    write.csv(
      iron_sig_res,
      file.path(FGSEA_IRON_TABLE_DIR, paste0("fgsea_HA_vs_other_", safe_ct, "_iron_related_significant.csv")),
      row.names = FALSE
    )
  }
  ...
  if (nrow(iron_sig_res) == 0) {
    cat("No iron-related GSEA curves saved for ", cell_type_here, ".\n", sep = "")
    next
  }
  ...
  for (j in seq_len(nrow(iron_sig_res))) {
    ...
    ggsave(
      filename = file.path(FGSEA_CURVE_DIR, paste0("gsea_curve_", safe_ct, "_", pathway_safe, ".png")),
      plot = p_curve,
      width = 8,
      height = 6,
      dpi = 600
...
if (length(all_sig_results) > 0) {
  ...
  write.csv(
    all_sig_df,
    file.path(FGSEA_RES_DIR, "fgsea_HA_vs_other_all_celltypes_significant.csv"),
    row.names = FALSE
  )
}
...
if (length(all_iron_sig_results) > 0) {
  ...
  write.csv(
    all_iron_sig_df,
    file.path(FGSEA_RES_DIR, "fgsea_HA_vs_other_all_celltypes_iron_related_significant.csv"),
    row.names = FALSE
  )
}

cat("\n============================================================\n")
cat("fgsea ranked GSEA complete.\n")
